...
class Advisor(nn.Module):
    def __init__(self, vocab_size, word_emb_size, hidden_size, \
        ...
        atten_type='add', score_type='clf', use_cuda=True, phase=2):
        ''' With masked dialog state.
        '''
        super(Advisor, self).__init__()
        print('[ Using the Advisor Mask model ]')
        ...
        # Mask
        self.linear_o = nn.Linear(hidden_size, hidden_size, bias=False)
        self.linear_cands = nn.Linear(hidden_size, hidden_size, bias=False)
        self.phase = phase
        # self.linear_o2 = nn.Linear(hidden_size, hidden_size, bias=False)
        self.linear_o3 = nn.Linear(hidden_size, hidden_size, bias=False)
    ...
    def forward(self, ctx, ctx_utter_len, ctx_turns_num, cands, cands_utter_len, db_fields, db_cells, db_rows_num, temperature=0.1, targets=None):
        # Data format
        ...
        # carr = F.normalize(db_row_mask, p=1, dim=-1)
        ...
        if self.phase == 1:
            # Phase 1)
            correct_answer = cands_vec.gather(1, targets.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, cands_vec.size(-1))).squeeze(1)
            mask_answer_part = self.linear_cands(correct_answer)

            phase_1_loss_mask_answer = torch.mean((self.linear_o3(last_o) - mask_answer_part) ** 2)
        else:
            # Phase 2)
            # u_k_1 = self.linear_o2(last_o)
            mask_answer_part = self.linear_o3(last_o)
            phase_1_loss_mask_answer = 0


        mask = F.sigmoid(self.linear_o(last_o) + mask_answer_part)
        last_o = last_o * mask
        ...
        return score, all_theta, all_acc, phase_1_loss_mask_answer
...
class Decision(nn.Module):
    def __init__(self, hidden_size, input_size, use_cuda=True):
        ...
        '''distribution over modes {recommend, query, inform, social}'''
        self.use_cuda = use_cuda
