...
from .modules import EncoderCNN, EncoderRNN, Attention, GRUStep, EncoderKnowledge, create_mask
from .utils import to_var

INF = 1e20
NUM_MODES = 4

class HRED_DB0(nn.Module):
    def __init__(self, vocab_size, word_emb_size, hidden_size, \
        ...
        and does not cumulatively update search space.
        '''
        super(HRED_DB0, self).__init__()
        print('[ Using the HRED_DB0 model ]')
    ...
    def forward(self, ctx, ctx_utter_len, ctx_turns_num, cands, cands_utter_len, db_fields, db_cells, db_rows_num, temperature=0.1):
        ...
        for i in range(0, ctx.size(1), 2):
            ...
            o, dialog_state, acc = self.hop(dialog_state, user_utter, pre_sys_utter, db_fields_vec, db_cells_vec, db_row_mask, temperature)
    ...
    def hop(self, dialog_state, user_utter, pre_sys_utter, db_fields_vec, db_cells_vec, db_row_mask, temperature=0.1):
        # Update dialog state
        dialog_state = self.update_dialog_state(dialog_state, user_utter, pre_sys_utter)

        # Update CARR
        arr, acr = self.arr_step(user_utter, db_fields_vec, db_cells_vec, db_row_mask)

        # Update ACC
        acc = self.acc_step(user_utter, db_fields_vec, acr) # field idx 4: course

        # Compute output state
        o = self.compute_output_state(dialog_state, db_cells_vec, arr, acc)
        return o, dialog_state, acc

    def update_dialog_state(self, dialog_state, x, y):
        dialog_state = self.gru_step(dialog_state, torch.cat([x, y], -1))
        return dialog_state

    def compute_output_state(self, dialog_state, db_cells_vec, arr, acc):
        B, N, K, _ = db_cells_vec.size()
        m = torch.bmm(arr.unsqueeze(1), db_cells_vec.view(B, N, -1)).view(B, K, -1)
...
class AttentionRowsRows(nn.Module):
    ...
    def forward(self, x, db_fields_vec, db_cells_vec, db_row_mask=None):
        ...
        arr = self.atten_arr(x, rows_cells_vec, atten_mask=db_row_mask) # B x N
